...
from tracker.util.painter import mask_painter 
...
class BaseTracker:
    def __init__(self, xmem_checkpoint, device, sam_model=None, model_type=None) -> None:
        ...
        cfg_dir = [s for s in sys.path if "tracker/config" in s][0]
        with open(os.path.join(cfg_dir, "config.yaml"), 'r') as stream: 
